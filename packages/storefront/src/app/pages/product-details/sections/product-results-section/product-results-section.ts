...
export class ProductResultsSection {
  ...
  readonly content = computed(() => this.product().translations[this.language()].result);

  updateFromPointer(event: PointerEvent): void {
    const target = event.currentTarget as HTMLElement;
    const rect = target.getBoundingClientRect();
    this.position.set(Math.max(0, Math.min(100, ((event.clientX - rect.left) / rect.width) * 100)));
  }
  updateFromKeyboard(event: KeyboardEvent): void {
    if (event.key === 'ArrowLeft') this.position.update((value) => Math.max(0, value - 5));
    if (event.key === 'ArrowRight') this.position.update((value) => Math.min(100, value + 5));
  }
}
