...
export class ProductReviewsSection {
  ...
  readonly reviews = computed(() => this.product().translations[this.language()].reviews);
  readonly visibleReviews = computed(() => this.expanded() ? this.reviews().userReviews : this.reviews().userReviews.slice(0, 4));
}
